...
describe 'Group API interactions', Teamweek::Api::Client, :vcr do
  ...
  it "retrieves a specific group" do
    group = client.fetch(Teamweek::Api::Group, {id: 175548})
    expect(group.map(&:name)).to match_array ["Group test"]
  end

  it "updates a group" do
    group_after = client.update(Teamweek::Api::Group, {id: 175531, name: "Renamed group"})
    expect(group_after.first.name).to eq "Renamed group"
  end

  it "removes a group" do
    groups_before = client.fetch(Teamweek::Api::Group)
    client.delete(Teamweek::Api::Group, {id: 175547})
    groups_after = client.fetch(Teamweek::Api::Group)
    expect(groups_before.map(&:id)).to include 175547
    expect(groups_after.map(&:id)).to_not include 175547
  end
